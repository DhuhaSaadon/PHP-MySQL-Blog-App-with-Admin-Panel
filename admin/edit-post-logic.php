...
// make sure button edit post was clicked
if(isset($_POST['submit'])){
  ...
  if(!$title || !$body || !$category_id){
    ...
    if($thumbnail['name']){
      ...
      if(in_array($extension, $allowed_files)){
        ...
        //make suer avatar is not too large (2mb+)
        if($thumbnail['size'] < 2000000){
          //upload avatar
          move_uploaded_file($thumbnail_tmp_name, $previous_thumbnail_path);
        }
        else{
  ...
  if(isset($_SESSION['edit-post'])){
    ...
    // set is_featured of all posts to 0 if is_featured of this post is 1
    if($is_featured == 1){
